...
public class ChatClientConfig {
    ...
    # Papel
    Você é o assistente virtual de RH da Hvogel Tecnologia Ltda., uma fábrica de software.
    Seu público são os colaboradores da empresa (engenharia, QA, DevOps, produto, UX e administrativo).
    ...
    # Escopo
    - Responda apenas dúvidas sobre políticas internas, benefícios, conduta,
    jornada de trabalho, férias, licenças e procedimentos de RH da Hvogel.
    - Se a pergunta for claramente fora desse escopo (ex.: dúvidas técnicas de
    programação, arquitetura de software, opiniões pessoais), explique gentilmente
    que você só trata de assuntos de RH e oriente o canal adequado.
    ...
    @Value("${spring.ai.chat.memory.redis.url:}")
    private String redisUrl;

    @Value("${spring.ai.chat.memory.redis.username:}")
    private String redisUsername;

    @Value("${spring.ai.chat.memory.redis.password:}")
    private String redisPassword;

    @Value("${spring.ai.chat.memory.redis.ssl:false}")
    private boolean redisSsl;

    @Value("${spring.ai.chat.memory.redis.time-to-live:PT30M}")
    private Duration redisTimeToLive;

    @Value("${spring.ai.chat.memory.redis.initialize-schema:true}")
    private boolean redisInitializeSchema;

    @Bean
    public RedisChatMemoryRepository redisChatMemoryRepository() {
        return RedisChatMemoryRepository.builder()
                .jedisClient(createRedisClient())
    ...
    private RedisClient createRedisClient() {
        if (StringUtils.hasText(redisUrl)) {
            return RedisClient.create(URI.create(redisUrl.trim()));
        }

        boolean needsAuthOrSsl = StringUtils.hasText(redisPassword)
